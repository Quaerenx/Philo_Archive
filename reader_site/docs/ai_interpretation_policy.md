...
The tracked prompt template registry lives in `reader_site/data/ai_prompt_templates.json`. The current defaults are `segment_interpretation_v2` and `sentence_translation_study_v4`; the quality stages use `sentence_translation_critic_v2` and `sentence_translation_revision_v2`. Older templates remain registered so saved prompt hashes stay traceable. The interpretation prompt labels direct textual claims separately from reasoned inference. The translation prompt keeps the Korean translation free of commentary, preserves logical relations, morphology, word families, imagery, unusual wording, and meaningful ambiguity before naturalness, and forbids unsupported technical, legal, theological, or philosophical substitution. Within those fidelity constraints it requires grammatically complete Korean, permits source-supported antecedent repetition when a literal pronoun would be opaque, and rejects source-order fragments and accidental literal-calque redundancy. Source excerpts are quoted data rather than instructions. Linguistic knowledge may help parse the supplied wording, but outside biography, doctrine, background facts, and prior translations may not be introduced as evidence.

Every new draft receives a fresh critic request containing only the approved policy, source context, and draft translation; translator commentary is deliberately absent. The critic may use `korean_readability` only for a material fragment, dangling relation, redundant calque, or opaque referent—not for stylistic preference. A `pass` result is saved without revision. Minor-only issues are saved as `needs_human_review` without automatic rewriting. Any major issue permits exactly one schema-constrained revision followed by one final critic request. The system never enters an open-ended repair loop. Critic or revision failures are preserved explicitly as `critic_error` with a caution instead of being represented as a pass.

Human-approved work terminology and style decisions live separately in `reader_site/data/translation_profiles.json`. The application only reads entries whose `approval_state` is `approved`; it rejects duplicate approved scopes and has no code path that automatically accumulates model suggestions into this registry. A profile may add an exact-sentence rule tied to one stable `sentence_id`: forbidden Korean fragments become deterministic critic issues, while an explicitly allowed fragment can suppress only a model issue aimed at that exact approved form. These rules are not applied to other sentences. Optional evaluator, approval-time, and goldset-case provenance remain outside the model-facing policy text. An empty registry means that the model must not invent a fixed work-level terminology rule.
