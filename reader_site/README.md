...
Use `python .\scripts\check_translation_goldset.py --require-complete` to reject pending human evaluations. Use `--require-passing` as the stricter release gate: it also rejects every evaluated case with a rubric dimension below the passing score. See `docs/translation_quality_evaluation.md`.
